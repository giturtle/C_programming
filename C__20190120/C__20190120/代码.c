//改进点： malloc申请动态内存处理通讯录人数
//		通讯录中人数个数是固定的，开辟定长内存没有使用开销太大，所以就想到了动态内存管理，相对于直接申请更加科学
//		23行telephone_directory结构体成员发生改变，引入容量和指针
//		32行 Init函数发生改变，初始化多个结构体成员的值，设置默认容量值
//		61行 对于Add函数数组已满的判定条件进行更改，自定制realloc函数对数组进行扩容(直接使用系统realloc函数也可以)
...
#include <assert.h>
#include <string.h>
#define CONTACT_MAX_SIZE 500  //最大联系人的宏定义，可以随时在这里修改影响程序
...
typedef struct telephone_directory {
	Contact* person;
	int capacity;	//容量，表示person指针指向的动态数组的长度(元素个数)
...
void Init(telephone_directory* telephone_directory) {	//这里通过指针传参，如果不使用指针，函数的形参就不会影响到实参
	...
	telephone_directory->capacity = 10;	//初始化为10，表示申请一个10个元素那么大的内存
	telephone_directory->person = (Contact*)malloc(sizeof(Contact) * telephone_directory->capacity);
	//这个结构体数组是通过 size 控制数组元素的有效个数
	//当把 size 设为 0 ，证明无论数组元素内容是什么都是无效的，等同于清空。
}

void Realloc(telephone_directory* telephone_directory) {
	assert(telephone_directory != NULL);
	//1. 扩大 capacity 的值
	telephone_directory->capacity *= 2;	//扩容程度自定制，本人选择扩大一倍

	//2. 申请一个更大的内存
	//telephone_directory->person = (Contact*)malloc(telephone_directory->capacity * sizeof(Contact));
	//上面这种写法容易造成 内存泄漏，所以要将原来申请的内存备份在新内存中，然后释放原内存
	Contact* old_person = telephone_directory->person;	//备份原申请内存
	telephone_directory->person = (Contact*)malloc(telephone_directory->capacity * sizeof(Contact));

	//3. 复制原内存中的数据
	for (int i = 0; i < telephone_directory->size; ++i) {
		telephone_directory->person[i] = old_person[i];
	}
...
void Add_Contact(telephone_directory* telephone_directory) {
	assert(telephone_directory != NULL);
	if (telephone_directory->size >= telephone_directory->capacity) {
		printf("当前通讯录已满,进行扩容\n");
		Realloc(telephone_directory);		//	自定制realloc函数对数组进行扩容
	}
	printf("\n\n******开始新增联系人:******\n");
